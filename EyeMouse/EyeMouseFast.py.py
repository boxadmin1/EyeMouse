...
def get_eye_landmarks(landmarks, w, h, left=True):
    # Return key eye landmarks for head pose & eyeball estimation
    if left:
        idxs = [33, 133, 159, 145, 27, 130]  # Left eye landmarks
    else:
        idxs = [362, 263, 386, 374, 257, 359]  # Right eye landmarks
    return [(landmarks.landmark[i].x * w, landmarks.landmark[i].y * h) for i in idxs]

def get_head_pose(landmarks, w, h):
    # 3D model points of facial landmarks for solvePnP
...
while True:
    ...
    if results.multi_face_landmarks:
        ...
        # Head pose compensation (basic)
        pose = get_head_pose(landmarks, w, h)
        if pose is not None:
            pitch, yaw, roll = pose
            # small adjustment factors
            pred_x += int(yaw * 5)
            pred_y += int(pitch * 5)
